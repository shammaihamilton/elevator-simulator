...
export class ElevatorManager implements IElevatorManager {
  ...
  constructor(elevators: IElevatorFSM[], cfg?: Partial<SelectionConfig>) {
    if (!elevators.length)
      throw new Error("ElevatorManager requires at least one car");
  ...
  private scoredMetric(
    ...
    if (car.queueContainsFloor(req.sourceFloor))
    ...
    const queuePenalty = (car.queue as any).length?.() // support Queue API or array‑like
      ...
      : car.queue.length() * queueWeightMs; // fallback to custom length()

    // 3 ▸ moving in wrong direction?
    const wrongDir =
      (car.state === ElevatorState.MOVING_UP &&
        car.getCurrentFloor() > req.sourceFloor) ||
      (car.state === ElevatorState.MOVING_DOWN &&
        car.getCurrentFloor() < req.sourceFloor)
        ? wrongDirPenaltyMs
        : 0;
  ...
  getElevatorStates(): ElevatorStateObject[] {
    const elevatorStates: ElevatorStateObject[] = this.elevators.map(
  ...
  getElevatorStatesByBuilding(buildingId: string): IElevatorFSM[] {
    return this.elevators.filter((elevator) =>
      elevator.id.startsWith(buildingId)
    );
  }
